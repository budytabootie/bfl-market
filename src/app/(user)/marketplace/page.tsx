...
export default function MarketplacePage() {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-4">
      ...
      <div className="shrink-0 rounded-2xl border border-slate-800/80 bg-linear-to-br from-slate-900/80 to-slate-950/60 p-6 ring-1 ring-slate-700/30">
        ...
        <Link href="/po" className="mt-3 inline-flex items-center gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-300 transition-colors hover:bg-amber-500/20">
          Lihat barang PO minggu ini →
        </Link>
      </div>
      <div className="shrink-0">
        <h2 className="text-sm font-medium text-slate-400 uppercase tracking-wider mb-4">Katalog</h2>
        <TableToolbar
      ...
      <div className="shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <Link href="/cart">
          <Button className="w-full sm:w-auto min-w-[130px] py-2 text-xs" variant={cartCount > 0 ? 'primary' : 'secondary'}>
      ...
      {paginatedItems.length === 0 ? (
        <div className="flex-1 min-h-0 flex items-center justify-center py-12">
          <p className="text-center text-slate-500">Tidak ada produk yang cocok. Coba ubah filter atau kata kunci.</p>
        </div>
      ) : (
      <div className="marketplace-grid-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden pr-2 -mr-2">
