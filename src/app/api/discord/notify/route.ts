...
async function sendDiscordDM(discordId: string, message: string): Promise<{ ok: boolean; error?: string }> {
  ...
  // Discord ID harus numeric snowflake
  ...
  if (!createChannelRes.ok) {
    ...
    const hint = DISCORD_ERROR_MESSAGES[code as keyof typeof DISCORD_ERROR_MESSAGES] ?? createErrData.message ?? createChannelRes.statusText;
  ...
  if (!sendRes.ok) {
    ...
    const hint = DISCORD_ERROR_MESSAGES[code as keyof typeof DISCORD_ERROR_MESSAGES] ?? sendErrData.message ?? sendRes.statusText;
